# Make_tablM7() monthly job tenure tables function
# May 28, 2021
...
Make_tablM7 <- function(geo,jtn,sex,type,month1,month2) {
  ...
  title <- "Job tenure by occupation"
  subtitle <- case_when(
        type==1 ~ if (jtn!="Average tenure") {
          paste0("Thousands of persons<br>",jtn,", ",geo,", ",sex,"<br>",
          TS[[7]]$Seas,"<br><br>")
        } else {
          paste0("Months<br>",jtn,", ",geo,", ",sex,"<br>",
          TS[[7]]$Seas,"<br><br>")
        },
  ...
  if(type==1) { # Display original data
    q1 <- filter(q0,GEO==geo,JTN==jtn,SEX==sex)
    q1 <- select(q1,REF_DATE,NOC,VALUE)
    q1 <- pivot_wider(q1,names_from=NOC,values_from=VALUE)
