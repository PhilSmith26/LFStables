...
mc5UI <- function(id) {
  tabPanel(tags$b(tags$span(style="color:blue", HTML("Charts"))),
    ...
    selectInput(NS(id,"noc"), tags$b(tags$span(style="color:blue;font-size:20px", 
      "Choose an occupational group:")),choices=noc501,width = "100%"),
    prettyRadioButtons(NS(id,"hrw"),tags$b(tags$span(style="color:blue;font-size:20px", 
        "Choose an hourly wage range:")),choices=hrw501,bigger=TRUE,
        outline=TRUE,inline=TRUE,shape="round",animation="pulse"),
    prettyRadioButtons(NS(id,"tow"),tags$b(tags$span(style="color:blue;font-size:20px", 
...
mc5Server <- function(id) {
  moduleServer(id,function(input,output,session) {
    ...
    output$downloadData1 <- downloadHandler(
      filename=function() {
        paste0(str_replace_all(hrw1()," ","_"),"_",
