...
mc6UI <- function(id) {
  tabPanel(tags$b(tags$span(style="color:blue", HTML("Charts"))),
    ...
    selectInput(NS(id,"haw"), tags$b(tags$span(style="color:blue;font-size:20px", 
      "Choose a table:")),choices=haw601,width = "100%"),
    tags$style(HTML(".selectize-input, .option {
      color:black; 
      font-size:26px;
      font-family:Optima
    }")), 
    selectInput(NS(id,"chr"), tags$b(tags$span(style="color:blue;font-size:20px", 
      "Choose a characteristic:")),choices=chr601,width = "100%"),
    fluidRow(column(6,
...
mc6Server <- function(id) {
  moduleServer(id,function(input,output,session) {
    ...
    output$downloadData1 <- downloadHandler(
      filename=function() {
        paste0(str_replace_all(haw1()," ","_"),"_",
