# Make_tablM4() monthly occupations tables function
# May 27, 2021
...
Make_tablM4 <- function(lfc,geo,sex,type,month1,month2) {
  ...
  subtitle1 <- paste0(lfc,", ",geo,", ",sex,"<br>")
  subtitle2 <- case_when(
        type==1 ~ paste0(TS[[4]]$Units,"<br>",
          TS[[4]]$Seas,"<br><br>"),
  ...
  if(type==1) { # Display original data
    q1 <- filter(q0,(REF_DATE>=month1 & REF_DATE<=month2),
      LFC==lfc,GEO==geo,SEX==sex)
    q1 <- select(q1,-LFC,-GEO,-SEX)
    q1 <- pivot_wider(q1,names_from=NOC,values_from=VALUE)
    if (lfc=="Unemployment rate") {dec1 <- 1} else {dec1 <- 0}
    ...
    q1 <- filter(q0,LFC==lfc,GEO==geo,SEX==sex)
    q1 <- select(q1,-LFC,-GEO,-SEX)
    q1 <- pivot_wider(q1,names_from=NOC,values_from=VALUE)
    q1 <- mutate(q1,across(2:ncol(q1),function(x) 
